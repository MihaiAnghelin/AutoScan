    ...
    function init_template() {
        ...
        function menuFunctions() {
            ...
            menu.each(function () {
                ...
                if ($(this).hasClass('menu-box-right')) { $('.menu').hide(0); $(this).css("width", menuWidth); }
                if ($(this).hasClass('menu-box-left')) { $('.menu').hide(0); $(this).css("width", menuWidth); }
            ...
            menuDeployer.on('click', function () {
                ...
                if (menuEffect === "menu-reveal") {
                    if (menuID.hasClass('menu-box-left')) { headerAndContent.css("transform", "translateX(" + (menuWidth) + "px)"); menuHider.css("transform", "translateX(" + (menuWidth) + "px)"); }
                    if (menuID.hasClass('menu-box-right')) { headerAndContent.css("transform", "translateX(" + (menuWidth) * (-1) + "px)"); menuHider.css("transform", "translateX(" + (menuWidth) * (-1) + "px)"); }
                }
                ...
                $('[data-menu="menu-main"]').on('click', function () { $('#menu-contact').hide(0); })
                $('[data-menu="menu-contact"]').on('click', function () { $('#menu-main').hide(0); })
            ...
            if (autoActivateMenu.length) {
                if ($(this).hasClass('menu-box-right')) { $(this).show(); }
                if ($(this).hasClass('menu-box-left')) { $(this).show(); }
        ...
        function removeTransitions() { $('body').append('<style id="transitions-remove">.btn, .header, .menu-box, #footer-bar, .menu-active{transition:all 0ms ease!important;}</style>'); }
        ...
        function darkMode() {
            ...
            $('[data-toggle-theme], [data-toggle-theme-switch]').off().on('click', function () {
                ...
                if ($('body').hasClass('theme-light')) {
                    ...
                    localStorage.setItem('DuoDrawer-Theme', 'light-mode')
                }
                if ($('body').hasClass('theme-dark')) {
                    darkSwitch.prop('checked', true);
                    localStorage.setItem('DuoDrawer-Theme', 'dark-mode')
                }
                setTimeout(function () { allowTransitions(); }, 150);
                return false;
            })
            if (localStorage.getItem('DuoDrawer-Theme') == "dark-mode") { darkSwitch.prop('checked', true); $('body').removeClass('theme-light').addClass('theme-dark'); }
            if (localStorage.getItem('DuoDrawer-Theme') == "light-mode") { darkSwitch.prop('checked', false); $('body').removeClass('theme-dark').addClass('theme-light'); }
        ...
        sessionStorage.setItem('z_DuoDrawer-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    sessionStorage.getItem('z_DuoDrawer-Visited:' + element.pathname) ||
                    sessionStorage.getItem('z_DuoDrawer-Visited:' + element.pathname + '/')) {
                    if (className) {
        ...
        $('.list-group a').on('click', function () { if ($(this).attr('href') != '#') { localStorage.setItem('z_DuoDrawer-Visited-Scroll-Position', $(window).scrollTop()); } })
        if ($('.check-visited').length) {
            setTimeout(function () {
                var getScroll = localStorage.getItem('z_DuoDrawer-Visited-Scroll-Position');
                $(window).scrollTop(getScroll)
                localStorage.removeItem('z_DuoDrawer-Visited-Scroll-Position');
            }, 200);
        }
        ...
        //Adding Local Storage for Visited Links
        localStorage.setItem('z_DuoDrawer-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    localStorage.getItem('z_DuoDrawer-Visited:' + element.pathname) ||
                    localStorage.getItem('z_DuoDrawer-Visited:' + element.pathname + '/')) {
                    if (className) {
                        $(element).addClass(className);
                        $(element).find('.badge').addClass('disabled');
        ...
        //Invoice
        if ($('.invoice').length) { $('.invoice').slideUp(0); }
        $('[data-invoice]').on('click', function () {
            var invoice = $(this).data('invoice');
            $('#' + invoice).removeClass('disabled').slideToggle(250);
        })
        ...
        if (visitDetection.length) {
            var lastVisitValue = localStorage.getItem('DuoDrawer-Last-Visited');
            var d = new Date();
            ...
            if (!lastVisitValue) {
                $('.never-visited').show();
                $('.before-visited').hide();
                localStorage.setItem('DuoDrawer-Last-Visited', '<br>Your last visit was ' + strDate + ' at ' + strTime)
            } else {
        ...
        setTimeout(function () {
            ...
            $('.store-slider').owlCarousel({ loop: true, margin: 0, nav: false, lazyLoad: true, items: 1, autoplay: true, autoplayTimeout: 4000 });
            ...
            $('.double-slider').owlCarousel({ loop: true, nav: false, items: 2, margin: 20, stagePadding: 20, dots: false });
            ...
            $('.task-slider, .collection-slider').owlCarousel({ loop: true, margin: 20, nav: false, stagePadding: 50, lazyLoad: true, items: 2, autoplay: false, autoplayTimeout: 4000 });
        ...
        function highlightLoader() {
            $('[data-highlight-style]').on('click', function () {
                ...
                localStorage.setItem('DuoDrawer-Highlight', getHighlight);
                $('#highlight').remove();
                $('head').append($('<link rel="stylesheet" id="highlight" type="text/css" />').attr('href', getHighlight));
                return false;
            });
            var rememberHighlight = localStorage.getItem('DuoDrawer-Highlight');
        ...
        function activate_upload_file() {
            ...
            $(".upload-file").change(function (e) {
                ...
                $('.ufload-file-data').removeClass('disabled');
        ...
        $('.input-required input[type="email"]').on('focusout', function () { if (emailValidator.test($(this).val())) { $(this).parent().find('em').html(validIcon); } else { if ($(this).val() === "") { $(this).parent().find('em').html("(obligatoriu)"); } else { $(this).parent().find('em').html(invalidIcon); } } });
        $('.input-required input[type="tel"]').on('focusout', function () { if (phoneValidator.test($(this).val())) { $(this).parent().find('em').html(validIcon); } else { if ($(this).val() === "") { $(this).parent().find('em').html("(obligatoriu)"); } else { $(this).parent().find('em').html(invalidIcon); } } });
        $('.input-required input[type="password"]').on('focusout', function () { if (passwordValidator.test($(this).val())) { $(this).parent().find('em').html(validIcon); } else { if ($(this).val() === "") { $(this).parent().find('em').html("(obligatoriu)"); } else { $(this).parent().find('em').html(invalidIcon); } } });
        $('.input-required input[type="url"]').on('focusout', function () { if (urlValidator.test($(this).val())) { $(this).parent().find('em').html(validIcon); } else { if ($(this).val() === "") { $(this).parent().find('em').html("(obligatoriu)"); } else { $(this).parent().find('em').html(invalidIcon); } } });
        $('.input-required input[type="name"]').on('focusout', function () { if (nameValidator.test($(this).val())) { $(this).parent().find('em').html(validIcon); } else { if ($(this).val() === "") { $(this).parent().find('em').html("(obligatoriu)"); } else { $(this).parent().find('em').html(invalidIcon); } } });
        $('.input-required textarea').on('focusout', function () { if (textareaValidator.test($(this).val())) { $(this).parent().find('em').html(validIcon); } else { if ($(this).val() === "") { $(this).parent().find('em').html("(obligatoriu)"); } else { $(this).parent().find('em').html(invalidIcon); } } });
        ...
        if (pwaEnabled === true) {
            ...
            var dismissDate = localStorage.getItem('DuoDrawer-PWA-Timeout-Value');
            if ((day - dismissDate) > pwaRemind) {
                localStorage.removeItem('DuoDrawer-PWA-Prompt')
            }

            //Dismiss Prompt Button
            $('.pwa-dismiss').on('click', function () {
                console.log('User Closed Add to Home / PWA Prompt')
                localStorage.setItem('DuoDrawer-PWA-Prompt', 'install-rejected');
                $('body').find('#menu-install-pwa-android, #menu-install-pwa-ios, .menu-hider').removeClass('menu-active');
                localStorage.setItem('DuoDrawer-PWA-Timeout-Value', day);
            });
            ...
            if (isMobile.Android()) {
                function showInstallPrompt() {
                    if ($('#menu-install-pwa-android, .add-to-home').length) {
                        if (localStorage.getItem('DuoDrawer-PWA-Prompt') != "install-rejected") {
                            setTimeout(function () {
